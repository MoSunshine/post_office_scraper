...
class scraper:
    ...
    def run(self,rerun):
        ...
        self.get_data_list(zip_code_list,rerun)
...
class dhl_scraper(scraper):
    ...
    """
    def get_data_list(self,zip_code_list,rerun):
        """
        ...
        for zip_code in zip_code_list["ZIP"]:
            ...
            driver = webdriver.Firefox(executable_path='..\webdriver\geckodriver.exe',options=options,service_log_path=os.path.devnull,firefox_profile=profile)
            try:
                ...
                for shop in shop_list:
                    ...
                    shop_id = shop.find_element_by_class_name("ListItemTemplate__ContentTitle-sc-j2blya-2").get_attribute('innerHTML') + street + zip_code + city
            ...
            result_frame.to_csv(path_or_buf='../results/shop_list_dhl.csv',index=False,encoding='utf-8')
...
class hermes_scraper(scraper):
    ...
    def get_data_list(self,zip_code_list):
        ...
        try:
            ...
            logger.info("No cookies needed maybe check.")
        time.sleep(1)
        driver.refresh()
        ###Start getting data###
        for zip_code in zip_code_list["ZIP"]:
            ...
            ###create output###
            result_frame = pd.DataFrame(result_dataset)
            result_frame.to_csv(path_or_buf='../results/shop_list_hermes.csv',index=False,encoding='utf-8')
        driver.quit() 
...
class gls_scraper(scraper):
    ...
    def get_data_list(self,zip_code_list):
        ...
        for zip_code in zip_code_list["ZIP"]:
            ...
            logger.infp("Get Data for zip code " + str(zip_code))
            ...
            ###create output###
            result_frame = pd.DataFrame(result_dataset)
            result_frame.to_csv(path_or_buf='../results/shop_list_gls.csv',index=False,encoding='utf-8')
...
class dpd_scraper(scraper):
    ...
    def get_data_list(self,zip_code_list):
        ...
        for zip_code in zip_code_list["ZIP"]:
            ...
            ###create output###    
            result_frame = pd.DataFrame(result_dataset)
            result_frame.to_csv(path_or_buf='../results/shop_list_dpd.csv',index=False,encoding='utf-8')